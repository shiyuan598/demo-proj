...
public class OpenApiConfig {
    ...
    public OpenAPI customOpenAPI() {
        return new OpenAPI()
            .info(new Info().title("Demo Project API").version("1.0").description("Demo Project API Documentation")
                .contact(new Contact().name("wangshiyuan").email("wangyuanhpu@163.com"))
                .license(new License().name("Apache 2.0").url("http://www.apache.org/licenses/LICENSE-2.0.html")))
            .components(new Components().addSecuritySchemes(SECURITY_SCHEME_NAME,
                new SecurityScheme().name(SECURITY_SCHEME_NAME) // 推荐使用常量，避免与上方不一致
    ...
    /** 根据请求路径动态决定是否在文档中显示 token 输入框 白名单路径将不会显示 Authorization 输入框 */
    @Bean
        ...
        return (Operation operation, HandlerMethod handlerMethod) -> {
            ...
            if (mappingInfo != null) {
                ...
                if (!paths.isEmpty()) {
                    ...
                    boolean isExcluded =
                        Arrays.stream(EXCLUDED_PATHS).anyMatch(pattern -> pathMatcher.match(pattern, path));

                    // 白名单不设置 security 字段（不显示 token），否则显示 token 输入框
                    operation.setSecurity(isExcluded ? Collections.emptyList()
